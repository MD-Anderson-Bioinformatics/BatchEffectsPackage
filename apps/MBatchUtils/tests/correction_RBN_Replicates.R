...
if (!is.null(getTestOutputDir()))
{
  ...
  myOrigConfigFile <- cleanFilePath(cleanFilePath(baseTestDir, "corrections"), "RBN_Replicates_config.tsv")
  dataDir <- cleanFilePath(dirname(baseTestDir), "MATRIX_DATA")

  data1file <- cleanFilePath(dataDir, "repl-iset-matrix.tsv")
  batch1file <- cleanFilePath(dataDir, "repl-iset-batches.tsv")
  data2file <- cleanFilePath(dataDir, "repl-vset-matrix.tsv")
  batch2file <- cleanFilePath(dataDir, "repl-vset-batches.tsv")

  myOutputDir <- cleanFilePath(baseOutputDir, "correction_RBN_Replicates")
