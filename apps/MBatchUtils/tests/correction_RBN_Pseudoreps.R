...
if (!is.null(getTestOutputDir()))
{
  ...
  myOrigConfigFile <- cleanFilePath(cleanFilePath(baseTestDir, "corrections"), "RBN_Pseudoreps_config.tsv")
  dataDir <- cleanFilePath(dirname(baseTestDir), "MATRIX_DATA")

  data1file <- cleanFilePath(dataDir, "pseudo-iset-matrix.tsv")
  batch1file <- cleanFilePath(dataDir, "pseudo-iset-batches.tsv")
  data2file <- cleanFilePath(dataDir, "pseudo-vset-matrix.tsv")
  batch2file <- cleanFilePath(dataDir, "pseudo-vset-batches.tsv")

  myOutputDir <- cleanFilePath(baseOutputDir, "correction_RBN_Pseudoreps")
