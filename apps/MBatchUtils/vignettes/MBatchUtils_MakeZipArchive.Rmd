...
```{r comment=''}
  # set the paths
  theConfigFile="/BatchEffectsPackage_data/testing_static/MBatchUtils/vignettes/MBatchConfig.tsv"
...
##mutBatchFlag	FALSE
TRUE or FALSE flag telling whether or not do do "mutBatch" processing on a single dataset in Standardized Data format.
...
##mutBatchMem	4000m
Memory to allocate to the Java process when mutBatch processing.
...
```{r, echo=TRUE}
{
  ...
  theOutputDirMBatch=file.path(theOutputDir, "MBatch")
  jarDir=file.path(baseTestDir, "exe")
  javaExe=getJava()
  jarFile=file.path(jarDir, "ShaidyMapGen.jar")
  theDestConfigFile=file.path(theOutputDir, "MBatchConfig.tsv")
  theDestGeneFile=file.path(theOutputDir, "matrix_data.tsv")
  theDestBatchFile=file.path(theOutputDir, "batches.tsv")
  ####
  ...
  # config
  mbatchRunFromConfig(theConfigFile=theDestConfigFile,
					  ...
					  theBoxplotMem="8G",
					  theRunPostFlag=TRUE)
  buildSingleArchive("none", NULL, theOutputDirMBatch, theOutputDirMBatch)
}
```
