...
def convert_scdiff(the_dataset: Dataset, the_sample_dir: str, the_download_dir: str) -> Tuple[pandas.DataFrame, List[str]]:
    """
    ...
    """
    print(f"convert_scdiff size={len(the_dataset.files)} for {the_dataset.get_dataset_path('/')}", flush=True)
    ...
    for my_datafile in the_dataset.files.values():
        ...
        # print(f"convert_scdiff zip_file={zip_file} file_name={my_datafile.file_name}", flush=True)
        my_datafile.read_sample_file(the_sample_dir)
        assert 1 == len(my_datafile.sample_dict), "convert_scdiff - only expected one sample"
...
def read_and_process_file(the_matrix: pandas.DataFrame, the_barcode: str,
    ...
    with zipfile.ZipFile(the_file_zip, 'r') as zip_file:
        with zip_file.open(the_filename, mode="r") as in_file:
            ...
            print(f"convert_scdiff {the_barcode}", flush=True, end='')
            counter: int = 0
            for line in io.TextIOWrapper(in_file, encoding="utf-8"):
                ...
                if not line.startswith('#'):
                    if headers is None:
                        ...
                        feature: str = tsv_dict['gene']
                        val_str: str = tsv_dict['avg_log2FC']
