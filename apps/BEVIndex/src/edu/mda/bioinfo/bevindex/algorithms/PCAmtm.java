...
public class PCAmtm
{
	...
	public PCAmtm(File theAlgDir, MBatchIndexData theMID)
	{
		mAlgDir = theAlgDir;
		mMID = theMID;
	}
	...
	public void checkAndAddAlgorithm()
	{
		if (mAlgDir.exists())
		{
			...
			{
				File mtmDir = new File(diaDir, "ManyToMany");
				if (mtmDir.exists())
				{
					DropdownEntry batchType = new DropdownEntry();
					batchType.entry_label = diaDir.getName();
					batchType.diagram_type = "pca";
					batchType.pca_annotations = "/PCA/" + diaDir.getName() + "/ManyToMany/PCAAnnotations.tsv";
					batchType.pca_values = "/PCA/" + diaDir.getName() + "/ManyToMany/PCAValues.tsv";
					File imageDir = new File(mtmDir, "PCA-Plus");
					...
					if (imageFile.exists())
					{
						batchType.diagram_image = "/PCA/" + diaDir.getName() + "/ManyToMany/PCA-Plus/ALL_Comp1_Comp2_Diagram.png";
					}
					if (legendFile.exists())
					{
						batchType.legend_image = "/PCA/" + diaDir.getName() + "/ManyToMany/PCA-Plus/ALL_Comp1_Comp2_Legend-ALL.png";
					}
