...
theCompareFile=cleanFilePath(compareDir, "AN_Unadjusted.tsv")
theRandomSeed=314
#myRandomSeed <- 314
#myTestSeed <- 42
theBatchType="TSS"
...
if (!is.null(inputDir))
{
  ...
  AN_Unadjusted(theBeaData=myData,
              ...
              theWriteToFile=TRUE)
  correctedMatrix <- readAsGenericMatrix(cleanFilePath(theOutputDir, "ANY_Corrections-ANUnadjusted.tsv"))
